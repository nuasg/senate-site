    ...
    if request.xhr?
      content_tag(:div,
                  content_tag(:div, class: 'voting') do
                    button_to('Aye', {controller: :documents, action: :vote, vote: :aye, id: doc.id, class: doc.get_user_vote(@user).vote == :aye ? 'selected' : ''}, {remote: true})
                    button_to('Nay', {controller: :documents, action: :vote, vote: :nay, id: doc.id, class: doc.get_user_vote(@user).vote == :nay ? 'selected' : ''}, {remote: true})
                    button_to('Abstain', {controller: :documents, action: :vote, vote: :abstain, id: doc.id, class: doc.get_user_vote(@user).vote == :abstain ? 'selected' : ''}, {remote: true})
                  end,
                  class: 'middle')
    else
      concat(content_tag(:div, class: 'middle') do
        concat(content_tag(:div, {class: 'voting'}) do
          concat(content_tag(:input, nil, remote: true, disabled: true, value: doc.get_user_vote(@user).vote.capitalize, type: 'submit'))
        end)
      end)
    end
  end
